...
- 同一站点只显示一条记录，批注分行列出签到和每日抽奖各自的结果、奖励；绿字“本次”奖励合并本轮两项所得（签到在更早轮次完成时显示“今日”合计），余额显示最终余额。抽奖不参与签到奖励的余额差复核。
...
在`skip.hosts`中填写站点域名关键字，按包含关系匹配；默认空清单不跳过任何站点。
...
命中的站点显示绿色“跳过签到”，不会提交签到或为查询启动浏览器验证，
但仍查询余额；查询失败时保留已有余额并标注“缓存”。
跳过不会改写真实签到日期和确认状态。

待签到数量与耗时估算排除跳过项，报告仍保留这些账号的余额记录。
例如14个账号中有3个跳过，进度显示11个待签到、另3个只查余额，报告仍有14条。

[跳过余额报告示例（模拟数据）](docs/images/skip-balance.png)
